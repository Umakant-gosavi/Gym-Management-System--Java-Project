...
private JTextField proname;
	public Sell_Product() {
		setTitle("Mass Gym || Sell Product");
		setResizable(false);
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setBounds(380, 150, 719, 483);
		...
		panel_2.setBounds(6, 262, 697, 149);
		...
		JLabel lblInformation = new JLabel("Date");
		lblInformation.setBounds(410, 87, 107, 19);
		...
		JLabel lblProductNames = new JLabel("Product Names");
		lblProductNames.setFont(new Font("Times New Roman", Font.PLAIN, 16));
		lblProductNames.setBounds(410, 59, 107, 19);
		panel_2.add(lblProductNames);
		
		proname = new JTextField();
		proname.setColumns(10);
		proname.setBounds(523, 60, 146, 20);
		panel_2.add(proname);
		
		JDateChooser selldate = new JDateChooser();
		selldate.setBounds(523, 87, 146, 20);
		panel_2.add(selldate);
		selldate.setDateFormatString("yyyy-MM-dd");
		
		Date ddd=new Date();
		selldate.setDate(ddd);
		...
		btnProceed.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				txtAMount2.setEnabled(false);
				proname.setText(txtpname.getText());
		...
		lblPleaseSureAll.setBounds(0, 426, 457, 14);
		...
		btnSell.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				{
					con=Connect.connectDb();
					String sql = "insert into sell_product(Customer_name,Product_name,Purchase_date,Amount,Discount,Total_amount,Paid_amount,Balance)values(?,?,?,?,?,?,?,?)";
					pst=con.prepareStatement(sql);
					
					if(!Pattern.matches("^[a-zA-Z]+(\\s[a-zA-Z]+)?$", txtCustomer_Name.getText()))		//Only Characters
			        {	
			        	JOptionPane.showMessageDialog(null, "Enter Only Charecter in Customer Name");
			        }
					else 
					{
					...
					if(!Pattern.matches("^[a-zA-Z]+(\\s[a-zA-Z]+)?$", proname.getText()))		//Only Characters
			        {	
					...
					pst.setString (3, ((JTextField)selldate.getDateEditor().getUiComponent()).getText());
					
					pst.setString(4, txtAMount2.getText());
					
					if(!Pattern.matches("^[0-9]+$", txtdiscount.getText()))		//Only Digits
			        {
		        	 JOptionPane.showMessageDialog(null, "Enter Only Number in Discount");
			        }
			        else
			        {
					...
					if(!Pattern.matches("^[0-9]+$", txtPaid.getText()))		//Only Digits
			        {
		        	 JOptionPane.showMessageDialog(null, "Enter Only Number in Discount");
			        }
			        else
					...
					pst.setString(8, txtChange.getText());
		...
		btnSell.setBounds(481, 422, 89, 23);
		...
		btnCancel.setBounds(591, 422, 106, 23);
